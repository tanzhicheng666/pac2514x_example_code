/**************************************************************************//**
 * @file     pac_driver_system.c
 * @brief    Firmware driver for the PAC system and clock controllers
 *
 * @note
...
RAMFUNC_SYSTEM void pac_sys_pll_config_enable(uint32_t pllindiv, uint32_t pllfbdiv, uint32_t plloutdiv)
{
    //  Configure PLL control reg and enable PLL
    PAC_SCC->CCSPLLCTL.PLLEN = 0;
    PAC_SCC->CCSPLLCTL.PLLFBDIV = pllfbdiv;
    PAC_SCC->CCSPLLCTL.PLLOUTDIV = plloutdiv;
    PAC_SCC->CCSPLLCTL.PLLINDIV = pllindiv;
    PAC_SCC->CCSPLLCTL.PLLEN = 1;
    // Wait for Lock
    while(PAC_SCC->CCSPLLCTL.PLLLOCK == 0);    
}


//=====================================================================================
// pac_recovery() provides a mechanism to recover a misconfigured PAC device
// Ground PE3 at power up to add a 6 second delay so that the Flash can be erased
//=====================================================================================
// NOTE:  #define PAC_RECOVERY should be commented out for a production system
// When PAC_RECOVERY is defined, then a 6 Second delay will be added if PE3 is connected to ground at PAC power up
...
#warning PAC_RECOVERY enabled; Ground PE3 to add 6 second boot delay to allow recovery of misconfigured device; remove for production system
...
void pac_recovery(void)
{
    ...
    PAC_SCC->PEPUEN.P3 = 1;                         // Enable Pull Up
    ...
    for (i=0; i < 40; i++)  // Value of 40 = at least 145 uS, which is enough for 1 UART packet (78uS)
    {
        ...
        if (PAC_GPIOE->IN.P3 == 1)
        {
            perform_delay = 0;
        }
    }

    // If PE3 was low, delay required, add 6 second delay
    if(perform_delay)
    {
        pac_delay_asm(2800000);
    }

    // Restore PE3 to reset state
    PAC_SCC->PEPUEN.P3 = 0;                         // Disable Pull Up
}
