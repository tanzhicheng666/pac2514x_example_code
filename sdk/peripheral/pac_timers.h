//=================================================================================
/// @file     pac_timers.h
/// @brief    CMSIS Cortex-M Header File for the PAC Timers
/// @note
...
#define VER1  //Define for original QEP definition
//=============================== Timer (A, B, C, D) ===================================
...
typedef enum {
	...
	TXCCTL_CCLATCH_CAPTURE_PERIOD_DUTY  = 3
...
//VER2 QEP Definitions
#ifndef VER1
// Define QEP Interrupt Flag Bits
#define QEP_DIRIF       (1 << 0)
#define QEP_PHAIF       (1 << 1)
#define QEP_PHBIF       (1 << 2)
#define QEP_WRIF        (1 << 3)
#define QEP_IDXEVIF     (1 << 4)
#endif
...
// PAC_Timer - TIMERA @ 0x40060000, TIMERB @ 0x40070000, TIMERC @ 0x40080000, TIMERD @ 0x40090000
typedef struct{     
...
// VER2 QEP Definitions    
    ...
    // QEPCTL (Timer QEP Control, Offset: 0x00300);  TIMERA @ 0x40060300, TIMERB @ 0x40070300, TIMERC @ 0x40080300, TIMERD @ 0x40090300
    union{
        __IO     uint32_t w;  
        struct {
            __IO uint32_t QEPEN             : 1;        // 0                QEP peripheral enabled    
            __IO uint32_t CNTAB             : 1;        // 1                Count on A/B    
            __IO uint32_t CNTEDGE           : 1;        // 2                Count on edge       
            __I  uint32_t DIR               : 1;        // 3                QEP direction   
            __IO uint32_t IDXRST            : 1;        // 4                Reset counter on index     
                 uint32_t RESERVED1         : 3;        // 5:7                     
            __IO uint32_t CLKDIV            : 3;        // 8:10             QEP Clock Divider
                 uint32_t RESERVED2         : 21;       // 11:31
        };
    } QEPCTL;
    
    __IO uint32_t QEPCTR;       // QEPCTR (Timer QEP Counter, Offset: 0x0304);  TIMERA @ 0x40060304, TIMERB @ 0x40070304, TIMERC @ 0x40080304, TIMERD @ 0x40090304
    __IO uint32_t QEPMAX;       // QEPMAX (Timer QEP Maximum Counter, Offset: 0x0308);  TIMERA @ 0x40060308, TIMERB @ 0x40070308, TIMERC @ 0x40080308, TIMERD @ 0x40090308

    // QEPIER (Timer QEP Interrupt Enable Register, Offset: 0x030C);  TIMERA @ 0x4006030C, TIMERB @ 0x4007030C, TIMERC @ 0x4008030C, TIMERD @ 0x4009030C    
    union{
        __IO     uint32_t w;  
        struct {
            __IO uint32_t DIRIE             : 1;        // 0                Direction change interrupt enable    
            __IO uint32_t PHAIE             : 1;        // 1                Phase A rising edge interrupt enable    
            __IO uint32_t PHBIE             : 1;        // 2                Phase B rising edge  interrupt enable       
            __IO uint32_t WRIE              : 1;        // 3                Index Event interrupt enable     
                 uint32_t IDXEVIE           : 1;        // 4                     
                 uint32_t RESERVED1         : 27;       // 5:31
        };
    } QEPIER;

    // QEPIFR (Timer QEP Interrupt Flag Register, Offset: 0x0310);  TIMERA @ 0x40060310, TIMERB @ 0x40070310, TIMERC @ 0x40080310, TIMERD @ 0x40090310        
    union{
        __IO     uint32_t w;  
        struct {
            __IO uint32_t DIRIF             : 1;        // 0                Direction change interrupt flag
            __IO uint32_t PHAIF             : 1;        // 1                Phase A rising edge interrupt flag
            __IO uint32_t PHBIF             : 1;        // 2                Phase B rising edge  interrupt flag
            __IO uint32_t WRIF              : 1;        // 3                Index Event interrupt flag
                 uint32_t IDXEVIF           : 1;        // 4                     
                 uint32_t RESERVED1         : 27;       // 5:31
        };
    } QEPIFR;
#endif
